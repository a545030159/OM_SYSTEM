...
text5_1 = document.getElementById("text5_1");
text6 = document.getElementById("text6");
text6_1 = document.getElementById("text6_1");
$textarea1 = $('#textarea1')
$textarea2 = $('#textarea2')
...
{
var fbutton = $(".filebutton_1")[0];//dom元素
//读取文件
var file = fbutton.files[0];
id = 1
startFileUpload(file);
...
function sendmessage(subject) {
     if (subject.trim()!=='') {
            ...
            if (id===0){
                ...
                xhr.open("POST", "/post_1/", false); //修改成自己的接口
...
{
    id = 0
    var subject = document.getElementById("textarea1").value;
    sendmessage(subject)
}

function openTextareaDialog_1()
{
    id = 1
    var subject = document.getElementById("textarea1_1").value;
    sendmessage(subject)
}
...
function viewFunction() {
     if(this.readyState === 4 && this.status === 200){
         if (id===0)
         {
             $('#textarea1').val(this.responseText);
         }
         else{
             $('#textarea1_1').val(this.responseText)
         }
        }
}
...
function uploadComplete(evt) {
    ...
    if (id===0){
        $('#textarea1').html(this.responseText);
         ...
         {
             $('#textarea1_1').html(this.responseText);
         }
}
...
function crawlers_1() {
    window.wxc.xcConfirm("目的地酒店名称","input",1)
                    }
...
function mycallback_1(data,now) {
            var endT = new Date();
            var mTime = (endT - now) / 1000;
            var Time = mTime.toFixed(2)
            var datalist_1 = JSON.parse(data);
            $('#get_time_1').text(Time+'s')
            sessionStorage.setItem('datalist_1',JSON.stringify(datalist_1))
            echarts0 = echarts.init(document.getElementById('echarts-dataset0_1'), 'walden');
            option0 = {
                    tooltip: {
                        trigger: 'axis',
                        axisPointer: {            // 坐标轴指示器，坐标轴触发有效
                    ...
                    series: [
                        {
                            ...
                            data: [Math.round(datalist_1.chart1['positive'] * 10000) / 100]
                        },
                        {
                            name: 'negative',
                            type: 'bar',
                            ...
                            data: [-Math.round(datalist_1.chart1['negative'] * 10000) / 100]
                        }
                    ]
                };
            echarts0.setOption(option0);
            echartsMap_1 = echarts.init(document.getElementById('echarts-dataset1_1'), 'walden');
            function generate_data() {
                    ...
                    $.each(datalist_1.chart2, function (k, v) {
                        data1.push(v.positive.toFixed(2));
                        data2.push((-v.negative).toFixed(2));
                        xAxisData.push(k);
                    })
            ...
            optionMap_1 = generate_data()
            echartsMap_1.setOption(optionMap_1);
            // echarts 窗口缩放自适应
            window.onresize = function () {
                    echarts0.resize()
                    echartsMap.resize();
                }
            echartsMap_1.getZr().off('click')
            echartsMap_1.getZr().on('click', params => {
                    let pointInPixel = [params.offsetX, params.offsetY]
                    if (echartsMap_1.containPixel('grid', pointInPixel)) {
                        let xIndex = echartsMap_1.convertFromPixel({seriesIndex: 0}, [params.offsetX, params.offsetY])[0]

                        var echartsMap2_1 = echarts.init(document.getElementById('echarts-dataset2_1'), 'walden');
                        ...
                        function generate_data() {
                            ...
                            var data2 = [];
                            var data3 = [];
                            chart_title_1 = optionMap_1.xAxis.data[xIndex];

                            $.each(datalist_1.chart3[optionMap_1.xAxis.data[xIndex]], function (k, v) {
                                data1.push(v.positive.toFixed(2));
                                data2.push((-v.negative).toFixed(2));
                                data3.push(v.rate)
                                xAxisData.push(k);
                        ...
                        optionMap2_1 = generate_data()
                        echartsMap2_1.setOption(optionMap2_1, true);
                        window.onresize = function () {
                            echartsMap2_1.resize();
                        }
                        echartsMap2_1.getZr().off('click');
                        echartsMap2_1.getZr().on('click', params => {
                            let pointInPixel = [params.offsetX, params.offsetY]
                            if (echartsMap2_1.containPixel('grid', pointInPixel)) {
                                let xIndex = echartsMap2_1.convertFromPixel({seriesIndex: 0}, [params.offsetX, params.offsetY])[0]
                                third_chart_title_1 = optionMap2_1.xAxis.data[xIndex]
                                evaluate_1 = datalist_1.chart3[chart_title_1][third_chart_title_1]
                                text5_1.innerHTML = "";
                                text6_1.innerHTML = "";
                                $('#title_pos_1').html(third_chart_title_1 + '的正面评价')
                                $('#title_neg_1').html(third_chart_title_1 + '的反面评价')
                                for (i = 0; i < evaluate_1['positive_instances'].length; i++) {
                                    var para = document.createElement('a');//创建新的p标签
                                    var para0 = document.createElement('div');//创建新的div标签
                                    var para1 = document.createElement('div');//创建新的div标签
                                    var para2 = document.createElement('div');//创建新的div标签
                                    var node0 = document.createTextNode(evaluate_1['positive_instances'][i][0]);//创建一个文本节点
                                    var node1 = document.createTextNode(evaluate_1['positive_instances'][i][2]);//创建一个文本节点
                                    var node2 = document.createTextNode(evaluate_1['positive_instances'][i][1].length);//创建一个文本节点
                                    para0.appendChild(node0);//向p追加此文本节点
                                    para1.appendChild(node1);//向p追加此文本节点
                                    para2.appendChild(node2);//向p追加此文本节点
                                    para0.setAttribute('style','display:inline-block;width: 40%')
                                    ...
                                    para.setAttribute("data-trigger","hover")
                                    para.setAttribute("title","用户观点：")
                                    para.setAttribute("data-content",evaluate_1['positive_instances'][i][1].join('<br>'))
                                    para.setAttribute("tabindex",i.toString())
                                    text5_1.appendChild(para);//向已有元素添加新元素（默认将para插入到element的最后）
                                }
                                for (i = 0; i < evaluate_1['negative_instances'].length; i++) {
                                    var para = document.createElement('a');//创建新的p标签
                                    var para0 = document.createElement('div');//创建新的div标签
                                    var para1 = document.createElement('div');//创建新的div标签
                                    var para2 = document.createElement('div');//创建新的div标签
                                    var node0 = document.createTextNode(evaluate_1['negative_instances'][i][0]);//创建一个文本节点
                                    var node1 = document.createTextNode(evaluate_1['negative_instances'][i][2]);//创建一个文本节点
                                    var node2 = document.createTextNode(evaluate_1['negative_instances'][i][1].length);//创建一个文本节点
                                    para0.appendChild(node0);//向p追加此文本节点
                                    para1.appendChild(node1);//向p追加此文本节点
                                    para2.appendChild(node2);//向p追加此文本节点
                                    para0.setAttribute('style','display:inline-block;width: 40%')
                                    ...
                                    para.setAttribute("data-trigger","hover")
                                    para.setAttribute("title","用户观点：")
                                    para.setAttribute("data-content",evaluate_1['negative_instances'][i][1].join('<br>'))
                                    para.setAttribute("tabindex",i.toString())
                                    para.setAttribute("data-placement","left")
                                    text6_1.appendChild(para);//向已有元素添加新元素（默认将para插入到element的最后）
                                }
                            }
                            $('[data-toggle="popover"]').popover({
                                html : true
                            })
                            $('#evaluation_pos_show_1').show()
                            $('#evaluation_neg_show_1').show()
                        }
                        )//end chartclick
                    }
                })
...
function show_result_1() {
    var datalist = JSON.parse(sessionStorage.getItem('datalist_1'))
    // if (datalist == null) return;
    var showtext = ""
    for (let i = 0; i < datalist['text1'].length; i++) {
        let text_o = datalist['text2'][i]['text']
...
function search_1() {
    var all_textarea_text = $textarea1_1.html();
    var selected_text = all_textarea_text.substr(0, window.getSelection().focusOffset);
    var current_line_num = selected_text.split('\n').length - 1;
    var arry = all_textarea_text.split("\n");
    var selected_line_content = arry[current_line_num] +'\n';
    $textarea1_1.html(all_textarea_text.replace(selected_line_content, '<div id = "selected_content_1" style="border:1px solid #f80707" onmouseout="clear_text_1()">'+selected_line_content+'</div>'));
    let selected_content = document.getElementById('selected_content_1')
    let move_dis = selected_content.offsetTop - selected_content.offsetHeight
    $textarea2_1.scrollTop( move_dis * 3 -  selected_content.offsetHeight)
    all_textarea_text = $textarea2_1.html();
    arry = all_textarea_text.split("\n");
    let selected_line_content_1 = arry[current_line_num * 3]
    let selected_line_content_2 = arry[current_line_num * 3 + 1]
    let selected_line_content_3 = arry[current_line_num * 3 + 2]
    selected_line_content = selected_line_content_1 + "\n" + selected_line_content_2+"\n"+selected_line_content_3 +"\n"
    $textarea2_1.html(all_textarea_text.replace(selected_line_content, '<div  style="border:1px solid #f80707">'+selected_line_content+'</div>'));
}
...
function clear_text_1() {
    ...
    var all_textarea_text = $textarea1_1.html();
    all_textarea_text = all_textarea_text.replace(reg,'');
    $textarea1_1.html(all_textarea_text)
    all_textarea_text = $textarea2_1.html();
    all_textarea_text = all_textarea_text.replace(reg,'');
    $textarea2_1.html(all_textarea_text)
}