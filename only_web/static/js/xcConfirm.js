...
(function($){
	...
	window.wxc.xcConfirm = function(popHtml, type, id_num) {
		id = id_num
		...
		var $txt1 = $("<p>").html(popHtml);//弹窗文本dom
		var $txt2 = $("<p>").html('关键字（选填）机场火车站酒店名称或区域');//弹窗文本dom
		...
		var $input1 = $("<input>").addClass("inputBox");//输入框
		var $input2 = $("<input>").addClass("inputBox");//输入框
		...
		function creatDom(){
			$popBox.append(
				...
				$txtBox.append($txt1).append($txt2)
			).append(
		...
		function doOk(){
			...
			$loading.show();
			progress([80, 90], [1, 3], 2000)  // 使用数组来表示随机数的区间
			if ($input1.is(":visible")&&$input2.is(":visible")) {
				doCancel()
				$.ajax(
					{
						type: "post",
						url: "/crawlers/",
						data: {'v1': v1, 'v2': v2, 'id': id},
						async: true,
						success:
							function (data) {
								if (id === 0) {
